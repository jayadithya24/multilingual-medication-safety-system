...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_function('window.scanSpeech.some(text => text.includes("Test medicine description"))')
        assert 'More details' not in page.evaluate('window.scanSpeech.at(-1)')
        assert 'Test warning' in page.evaluate('window.scanSpeech.at(-1)')
        ...
        page.wait_for_function("window.scanSpeech.length > 0")
        assert 'Metformin. 500 mg. After food' in page.evaluate('window.scanSpeech[0]')
        before_replay = page.evaluate('window.scanSpeech.length')
        page.get_by_role('button', name='Read scan aloud again').click()
        page.wait_for_function('(previous) => window.scanSpeech.length > previous', arg=before_replay)
